...
\item \textbf{ChrX} ia a logical value indicating whether the CpGs that belong to chromosome X should be deleted from normalization and the following analyses. The default is FALSE;
\end{itemize}
...
* \textbf{BetaAverage table} (it is compulsory that the file name contains the word "AvgBeta", case sensitive, and not the other reserved words)
